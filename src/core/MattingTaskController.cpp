...
MattingTaskController::MattingTaskController(QObject *parent)
    : QObject(parent)
{
    qRegisterMetaType<MattingResult>();
}

MattingTaskController::~MattingTaskController()
{
    if (thread_ != nullptr) {
...
bool MattingTaskController::isRunning() const noexcept
{
    return running_;
}

void MattingTaskController::start(const QString &imagePath, const QString &modelPath)
{
    if (running_) {
        emit failed(QStringLiteral("已有人物抠图任务正在运行"));
        return;
    }
...
void MattingTaskController::cancel()
{
