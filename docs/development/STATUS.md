...
- 当前节点：M0.2 — 目标驱动编辑
- 下一任务：完成 M0-04，对已选对象执行删除和移动；恢复原位置，比较传统 Inpainting 与生成式补全，并记录复杂背景伪影、等待时间和回退行为。
- 最近完成：M0-03；Qt 桌面端通过 MODNet + ONNX Runtime CPU 输出真实连续 Alpha，可在棋盘格预览并用画笔补回/擦除。人物/头发可行，简单猫狗样例仅作超范围观察，透明玻璃瓶明确失败。
...
- M0-03 使用 MODNet 官方仓库提供的 25,888,640-byte ONNX 人物模型，SHA-256 为 `07c308cf…6584df9`，官方仓库声明 Apache-2.0；模型只承诺人物抠图，不能因简单猫狗样例有效而扩展产品承诺。
- 透明玻璃不能由当前 Mask R-CNN + MODNet 组合解决；后续需评估许可证可接受的透明材质专用模型。缺少获准真值集时，只报告模型间差异与人工复核，不报告伪造准确率。
...
- M0-03 真实推理：`make evaluate-m0-03` 在 5 个固定案例上完成 Mask R-CNN 与 MODNet CPU 推理，技术错误为 0；人物头发、婚纱、猫毛、狗毛、透明玻璃的软透明像素占比分别为 7.37%、5.39%、21.58%、25.80%、0.02%。
- M0-03 人工复核：人物 Alpha 保留了二值 Mask 没有的发丝/羽毛软边；简单猫狗边缘可用但模型范围不匹配；透明玻璃完全失败。原始结果在被忽略的 `poc-results/*-m0-03/`，结论见 `M0_03_RESULTS.md`。
- M0-03 手工修正：单元测试验证 Alpha 软画笔补回/擦除、棋盘格合成和原图不变；Qt 界面测试验证鼠标实际修改 Alpha。固定画笔只能证明可行，产品化仍需缩放、参数和 Undo/Redo。
- 自动化验证：7 个 CTest 全部通过，其中 Mask R-CNN 与 MODNet 两项加载真实 ONNX 权重；当前开发任务进度为 4/193，下一项定位到 M0-04。

## 备注

- 当前 Qt POC 已加载真实实例分割和人物 Matting 模型；69.2% 对象探针结果低于最终普通对象 90% GO 阈值，透明玻璃仍失败，后续节点必须继续处理模型覆盖与边缘质量，不能提前给出 M0 `GO`。
